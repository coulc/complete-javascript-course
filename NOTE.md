...
# API(Application Programming Interface)
    API 是软件系统之间交互的桥梁,定义了如何请求数据或服务
    在前端开发中,通常指 Web API(通过 HTTP 协议访问的接口)

# AJAX(Asynchronous JavaScript and XML)
    一种技术,允许网页 异步 与服务器交换数据并更新部分页面内容,无需刷新整个页面
...
# PROMISES(ES6)
    Promise: 
        ...
        再或者: 未来值的占位符或容器
    好处:
        我们不再需要依赖传递到异步函数中的事件和回调来处理异步结果
        我们可以通过链式调用Promise来处理一系列异步操作,而无需嵌套回调,从而摆脱回调地狱

    lifecycle(生命周期):
    状态	        描述	                                    触发条件
    Pending	        初始状态,表示异步操作尚未完成	            Promise 被创建时默认状态
    Fulfilled	    操作成功完成,返回结果值(Resolved)  	    调用resolve(value)
    Rejected	    操作失败,返回错误原因	                    调用reject(error)
    状态在获取一次之后,无法改变

# 异步javascript在幕后是如何工作的?
    JavaScript 是单线程的,但它却能同时处理成千上万的异步任务
    这背后的秘密就是事件循环(Event Loop)
    事件循环就像一位高效的时间管理大师,它通过巧妙的调度机制,让JavaScript 在单线程环境下也能实现非阻塞的异步操作

    - 事件循环的基本概念:
        1.调用栈(Call Stack):
            调用栈是 JavaScript 执行同步代码的地方,它是一个后进先出(LIFO)的数据结构,用来记录函数的调用顺序
            每当一个函数被调用时,它就会被推入调用栈;当函数执行完毕后，它就会从调用栈中弹出。
        
        2.任务队列(Task Queue):
            任务队列用于存放异步任务的回调函数
            当异步任务(如 setTimeout,setInterval)完成时,它们的回调函数会被放入任务队列中，等待事件循环处理
        ...
        4.微任务队列(Microtask Queue)
            ...
            如:Promise的回调then,catch,finally

    - 事件循环的工作流程:
        执行同步代码:   从调用栈中执行同步任务
        清空微任务队列: 执行所有微任务(如 Promise 的回调)
