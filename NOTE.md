...
# OOP
    Object-oriented programming,一种基于对象概念的便成范式
- 范式: 一种编写代码和组织代码的风格
- 我们使用对象来建模(描述)现实世界的抽象特征,数据和相应的行为会打包到一个块中,
  对象是自身包含的代码段/代码块,
  对象是应用程序的构建块,它们彼此交互,
  交互是通过公共接口(public interface,API)发生的:对象外部的代码可以访问并用来与对象进行通信的方法。
  面向对象编程(OOP)的开发目标是组织代码,使其更加灵活且易于维护
- class(类):
    就像一张蓝图,我们可以根据它来创建新的对象,蓝图是一套抽象的规则
- 面向对象程序设计的4个基本原则
  抽象:  
    忽略或隐藏那些无关紧要的细节,使我们能够从宏观角度审视我们正在实现的事物,
    而不是纠结于那些对我们的实现并不重要的细节
  封装:
    在类内部保持属性和方法的私有性,这样它们就不能从类的外部访问,
    有些方法可以被暴露为公共接口(API)
  继承:
    将某个类的所有属性和方法都提供给子类使用,从而在类之间形成一种层次关系
    这使我们能够重用通用逻辑,并且能够模拟现实世界中的关系。 
  多态:
    子类可以覆盖从父类继承的方法
- prototypes(原型)
    每个JavaScript对象都有一个原型对象,用于实现继承和共享属性和方法
